...
const UploadDialog = ({ open, onClose, onCreated, theme }) => {
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    try {
      const res = await API.post('/api/skill/', form);
...
const SkillMarket = () => {
  ...
  const loadSkills = useCallback(async () => {
    setLoading(true);
    try {
      const [skillsRes, catRes, userRes] = await Promise.allSettled([
        API.get('/api/skill/?p=0'),
  ...
  return (
    <Box>
      ...
      <UploadDialog open={uploadOpen} onClose={() => setUploadOpen(false)} onCreated={loadSkills} theme={theme} />
