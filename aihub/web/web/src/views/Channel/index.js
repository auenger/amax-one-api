  ...
  return (
    <>
      <Stack direction="row" alignItems="center" justifyContent="space-between" mb={2.5}>
        <Typography variant="h3">渠道管理</Typography>
        <Button variant="contained" color="primary" startIcon={<IconPlus size={18} />} onClick={() => handleOpenModal(0)} sx={{ borderRadius: `${theme.customization.borderRadius}px` }}>
          新建渠道
        </Button>
      </Stack>
      <Card
      ...
      >
        <Box component="form" onSubmit={searchChannels} noValidate sx={{ pt: 2, px: 2 }}>
        ...
        >
          <Container>
            {matchUpMd ? (
              <ButtonGroup variant="outlined" aria-label="outlined small primary button group" sx={{ mb: 2, '& .MuiButton-root': { borderRadius: `${theme.customization.borderRadius}px`, borderColor: theme.palette.divider } }}>
                <Button onClick={handleRefresh} startIcon={<IconRefresh size={18} />}>
                  刷新
                </Button>
                <Button onClick={testAllChannels} startIcon={<IconBrandSpeedtest size={18} />}>
                  测试启用渠道
                </Button>
                <Button onClick={deleteAllDisabledChannels} startIcon={<IconHttpDelete size={18} />}>
                  删除禁用渠道
                </Button>
              </ButtonGroup>
            ) : (
